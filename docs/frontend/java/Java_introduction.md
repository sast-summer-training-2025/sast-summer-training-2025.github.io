...
Java中算数运算符和逻辑运算符与C语言中基本相同，这里不做赘述；特别需要注意的是逻辑右移 `>>>`和等于`==`。
...
在 Java 中，`Object` 类是所有类的基类，也就是说，每一个类（无论是标准库的还是用户自定义的）都直接或间接继承自 `java.lang.Object`。`Object` 类提供了一些基本方法，这些方法必须在子类中正确实现，以确保正确处理“判断两个对象是否相等”这一问题。
...
- 使用 `.equals()` 比较数值是否相等；
- 使用 `==` 会比较**引用地址**是否相同（128 以内的整数会被缓存）：
